...
public class LoginController implements CommunityConstant {
    ...
    public String register(Model model, User user){
        ...
        if (map==null||map.isEmpty()){//注册成功
            model.addAttribute("msg","注册成功，已经向您的邮箱发送激活邮件，请尽快激活账号");
    ...
    /**
     * 激活。为了和接口CommunityConstant中的状态对比，让本类实现该接口。
     */
    ...
    public String activation(Model model, @PathVariable("userId") int userId,@PathVariable("code") String code){
        ...
        if(res == ACTIVATION_SUCCESS){
            ...
            model.addAttribute("target","/login");//跳转登陆页面
        }else if(res == ACTIVATION_REPEAT){
            model.addAttribute("msg","无效操作，该账号已经激活过了！");
            model.addAttribute("target","/index");//跳转首页
        }else{
            model.addAttribute("msg","激活失败，提供的激活码错误");
            model.addAttribute("target","/index");//跳转首页
        }
        return "site/operate-result";
    }
    ...
    public String login(String username, String password,String code,boolean rememberme,
        ...
        //判断账号密码,以及是否登录成功并跳转到对应界面
        int expiredSecond = rememberme ? REMEBER_EXPIRED_SECOND : DEFAULT_EXPIRED_SECOND;
        Map<String, Object> map = userService.login(username, password, expiredSecond);
        if (map.containsKey("ticket")){
            Cookie cookie = new Cookie("ticket",map.get("ticket").toString());
            cookie.setPath(contextPath);//让这个cookie在整个项目都有效
            cookie.setMaxAge(expiredSecond);//cookie时间
